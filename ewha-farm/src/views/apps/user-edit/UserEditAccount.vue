  ...
  <div id="user-edit-tab-info">
    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 w-full">
        ...
        <vs-input class="w-full mt-4" label="이름" v-model="data_local.name" v-validate="'required|alpha_num'"
          name="name" />
        <span class="text-danger text-sm" v-show="errors.has('name')">{{ errors.first('name') }}</span>
        
        <vs-input class="w-full mt-4" label="핸드폰번호" v-model="data_local.mobile" type="mobile_phone"
          v-validate="'required|numeric'" name="email" />
      ...
      <div class="vx-col md:w-1/2 w-full">
        ...
        <vs-input class="w-full mt-4" disabled label="아이디" v-model="data_local.username" v-validate="'required|alpha_num'"
          name="username" />
        <span class="text-danger text-sm" v-show="errors.has('username')">{{ errors.first('username') }}</span>

        <vs-input class="w-full mt-4" label="농장 주소" v-model="data_local.country" v-validate="'required|alpha_spaces'"
          name="country" />
        <span class="text-danger text-sm" v-show="errors.has('country')">{{ errors.first('country') }}</span>

        <div class="mt-4">
          <label class="vs-input--label">권한</label>
          <v-select v-model="role_local" :clearable="false" :options="roleOptions" v-validate="'required'" name="role"
            :dir="$vs.rtl ? 'rtl' : 'ltr'" />
        ...
        <table class="w-full">
          <tr>
            ...
            <th class="font-semibold text-base text-left px-3 py-2"
              v-for="heading in ['Module', 'Read', 'Write', 'Create', 'Delete']" :key="heading">{{ heading }}</th>
          </tr>
